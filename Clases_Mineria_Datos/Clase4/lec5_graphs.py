...
def feature_target():
    ...
    ax.text(1, -1, r'Matriz Atributos ($X$)', size=14)
    ax.text(0.1, -0.3, r'cantidad (p)$\longrightarrow$', **font_properties)
    ax.text(-0.1, 0.1, r'$\longleftarrow$ tamaño_muestral',
            rotation=90, va='top', ha='right', **font_properties)

    ax.vlines(range(8, 10), ymin=0, ymax=9, lw=2, color='tomato')
    ax.hlines(range(10), xmin=8, xmax=9, lw=2, color='tomato')
    ax.text(7, -1, r'Vector Objetivo ($y$)', size=14)
    ax.text(7.9, 0.1, r'$\longleftarrow$ tamaño_muestral',
            rotation=90, va='top', ha='right', **font_properties)
...
def crossvalidation(cv_folds, ax, text_properties={}):
    ...
    for i in cv_folds:
        ...
        ax.add_patch(plt.Rectangle((5. * i / cv_folds, i),
                     5. / cv_folds, 0.7, fc='skyblue'))
        ax.text(5. * (i + .5) / cv_folds, i + .35, "Validación",
                ha='center', va='center', **text_properties)
        ax.text(0, i + .35, r'Ensayo {0}$\in${1}    '.format(cv_folds -
                i, cv_folds), ha='right', va='center', **text_properties)
        ax.text(5, i + .35, r'$f(\hat\theta - \Theta)\longrightarrow\varepsilon$',
                ha='left', **text_properties)
    ax.set_xlim(-1, 6)
...
def validate_curve():
    ...
    plt.xlim(0, 1)
    plt.fill_between(xaxis, -.6, .6, facecolor='grey',
                     alpha=.35, where=xaxis < .2)
    plt.text(0.05, 0.45, "Sesgo\nAlto", size=14)
    plt.fill_between(xaxis, -.6, .6, facecolor='grey',
                     alpha=.35, where=xaxis > .8)
...
def learning_curve():
    ...
    plt.fill_between(xaxis, -.6, 1.2, facecolor='grey',
                     alpha=.35, where=xaxis < .2)
...
def draw_rects(cv_folds, ax, text_properties={}):
    ...
    for i in range(cv_folds):
        ...
        ax.text(
            5, i+.35, r'$f(\hat\theta - \Theta)\longrightarrow\varepsilon_{{}}$', ha='left', **text_properties)
...
def train_testing():
    ...
    ax.text(-0.1, 0.1, 'testing_features', rotation=90,
            va='top', ha='right', **font_prop)
    ax.text(-0.1, 10, 'training_features', rotation=90,
            va='top', ha='right', **font_prop)
    ...
    ax.text(7.9, 0.1, 'testing_target', rotation=90,
            va='top', ha='right', **font_prop)
    ax.text(7.9, 10, 'training_target', rotation=90,
            va='top', ha='right', **font_prop)
...
def gauss_markov():
    """
    Draw Gauss-Markov theorem illustration
    """
